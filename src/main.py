...
from runtime import AgentTurnResult, resume_turn, format_interrupt, start_turn
...
def ask_yes_or_no (name: str) -> bool:
    while True:
...
def prompt_decision (pending: dict[str, Any]) -> list[dict]:
    ...
    for action in requests:
        ...
        if approved:
            ...
            decisions.append ({
                "type": "reject",
                "message": (
                    f"User decline {name}. Do not immediately retry the same"
...
def drain (agent, result: AgentTurnResult, config: dict) -> AgentTurnResult:
    while result.pending_interrupt is not None:
        decision = prompt_decision (result.pending_interrupt)
...
def chat () -> None:
    ...
    while True:
        ...
        try:
            result = drain (agent, start_turn (agent, user_input, config), config)
